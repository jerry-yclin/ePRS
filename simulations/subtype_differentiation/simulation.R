...
#
# The key comparison is between standard Elastic Net and ePRS to see which
# method can better learn to distinguish A from B.
...
for (sim in 1:sim_count) {
  ...
  for (iter in 1:iter_count) {
    ...
    source_a_risk <- df_source %*% beta_a + rnorm(n_row * 8, 0, 20)
    source_b_risk <- df_source %*% beta_b + rnorm(n_row * 8, 0, 20)
    target_a_risk <- df_target %*% beta_a + rnorm(n_row, 0, 20)
    target_b_risk <- df_target %*% beta_b + rnorm(n_row, 0, 20)
    ...
    subtype_test <- ifelse(diff_test > 0.5*sd(diff_test), 1,  # Subtype A
                           ifelse(diff_test < -0.5*sd(diff_test), 0, -1)) # Subtype B, -1 is ambiguous
    
    test_set_indices <- which(subtype_test != -1)
    ...
    ##-- Method 1: Standard Elastic Net (EN) --##
